...
parser = argparse.ArgumentParser(description='Codigo para realizar SfM (Structure from Motion)')
parser.add_argument('--images_folder', help='Path de la carpeta de imágenes', default='./images')
...
for (i, img_i) in enumerate(list(images.values())):
    for (j, img_j) in enumerate(list(images.values())[i+1::]):
        # Encontrar los match
        matches = matchWithRatioTest(matcher, img_i.descriptors, img_j.descriptors)

        # Test de reciprocidad
        matchRcp = matchWithRatioTest(matcher, img_j.descriptors, img_i.descriptors)
        ...
        for mR_m in matchRcp:
            ...
            for m_m in matches:
                ...
                if(mR_m.queryIdx == m_m.trainIdx and mR_m.trainIdx == m_m.queryIdx):
        ...
        F, inliersMask = cv.findFundamentalMat(np.array([img_i_pts]), np.array([img_j_pts]), cv.FM_RANSAC)
        ...
        if (float(len(final)) / float(len(matches)) < PAIR_MATCH_SURVIVAL_RATE):
            print("El match final tiene menos de la mitad de matches que el original. Ignorando imagen.")
            continue
        
        images[img_i.img_name].matches[(img_i.img_name,img_j.img_name)] = final
